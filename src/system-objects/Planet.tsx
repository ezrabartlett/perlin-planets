...
export default function Planet(props: PlanetProps) {
    ...
    useFrame((state, delta) => {
        if (meshRef.current) {// && props.colorProfile===0) {
            ...
            meshRef.current.position.z = pos[1];
    ...
    return (
        <>
            <mesh visible={true} ref={meshRef} onClick={handleCLicked} position={position}>
                ...
                <PlanetGeometry baseTemperature={baseTemperature} radius={radius} resolution={resolution} seed={props.attributes.seed} meshRef={meshRef} colorProfile={props.colorProfile} />
                <mesh >
